...
void setup(void)
{
  // pinMode(MCU_INT, OUTPUT);

  // Serial.println(digitalRead(MCU_INT));
  Serial.begin(9600);

  // attachInterrupt(digitalPinToInterrupt(MCU_INT), test, CHANGE);
  // Serial.print(F("Hello! ST77xx TFT Test"));
  while (!Serial)
    delay(10); // will pause Zero, Leonardo, etc until serial console opens

  Serial.println("--------MODULE CHECKS---------");
  if (!mpu.begin())
  {
  ...
  // // tft.fillScreen(ST77XX_BLACK); // fill screen with black (resets screen)
  // // time = millis() - time;
